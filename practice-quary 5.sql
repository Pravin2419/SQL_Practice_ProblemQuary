...
select FIRST_NAME as "Frist_Name",
...
-------------------------------------------------------------------------------------------------------------------------------------------------
#Write a query to display the last name, job, and salary for all employees whose job is that of a Programmer or a
...
----------------------------------------------------------------------------------------------------------------------------------------------------------
##########ggregate Functions and Group by########
...
select max(salary) -min(salary) as difrance_salary
from employees;
...
select JOB_ID, sum(salary) as t_salary,
min(salary) as m_salary,
avg(salary) as v_salary
FROM EMPLOYEES
where department_id ='90'
group by job_id;
